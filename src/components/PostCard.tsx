  ...
  return (
    <div className="flex py-12 space-x-5 items-center">
      ...
      <div className="flex-col space-y-2">
        <div className="bg-gray-300 h-7 w-80 rounded-md"></div>
        <div className="flex space-x-3">
          {[1,2,3].map((el) =>{
            return(
              <div key={el} className="h-6 w-12 bg-gray-300 rounded-md"></div>
            )
          })}
        </div>
        <div className="h-6 w-[852px] bg-gray-300 rounded-md"></div>
      </div>
    </div>
  )
}
    ...
    <div className="flex py-12 space-x-5 items-center border-t">
      ...
      <div className="flex-col">
        <Link to={`/post/${post.id}`}>
          <p className="text-xl font-bold hover:text-[#DC2777] inline">{post.title}</p>
        </Link>
        <div className="flex space-x-3">
          {post.tags.map((tag, index) =>(
            <button key={index} className="capitalize text-[#DC2777] hover:text-[#aa2762]">{tag}</button>
          ))}
        </div>
